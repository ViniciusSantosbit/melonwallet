 ...
 */
export function getBoundingBoxFromBBox(bbox) {
    return new BoundingBox([bbox.xMin, bbox.yMin], [bbox.xMax, bbox.yMin], [bbox.xMax, bbox.yMax], [bbox.xMin, bbox.yMax]);
}
/**
 * Given 2 bbox, merge them.
 */
export function mergeBbox(bbox1, bbox2) {
    return new BBox(Math.min(bbox1.xMin, bbox2.xMin), Math.min(bbox1.yMin, bbox2.yMin), Math.max(bbox1.xMax, bbox2.xMax), Math.max(bbox1.yMax, bbox2.yMax));
}
/**
...
export function getBbox(polygon) {
    ...
    return new BBox(Math.min(...allX), Math.min(...allY), Math.max(...allX), Math.max(...allY));
}
/**
 * Given polygons, calculate a BBox that encompasses all points.
 */
export function getBBoxForPolygons(polygons) {
